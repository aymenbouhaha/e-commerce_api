...
export class BasketService {
  ...
  async removeFromBasket(user: Partial<UserEntity>, productId: number) {
    ...
    try {
      ...
      throw new ConflictException(
        'Une erreur est survenue lors du supression du produit',
      );
    }
  }
  ...
  async addToBasket(
    ...
    productToBasket: ProductOrderDto,
  ) {
    if (user.role != UserRole.client) {
      throw new UnauthorizedException();
    }
    ...
    if (productToBasket.itemsNumber > product.itemsNumber) {
      throw new ConflictException(
        "La quantité restantes dans le stock n'est pas suffisantes",
      );
    }
    ...
    try {
      ...
      throw new ConflictException(
        "Le produit n'est pas ajouté au panier , Veuillez réessayer !",
      );
    }
  }
}
